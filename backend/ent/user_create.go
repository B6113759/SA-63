...
func (uc *UserCreate) AddDeceasedreceifeIDs(ids ...int) *UserCreate {
	uc.mutation.AddDeceasedreceifeIDs(ids...)
	return uc
}

// AddDeceasedreceives adds the deceasedreceives edges to DeceasedReceive.
func (uc *UserCreate) AddDeceasedreceives(d ...*DeceasedReceive) *UserCreate {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return uc.AddDeceasedreceifeIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.DeceasedreceivesIDs(); len(nodes) > 0 {
