...
func (tx *Tx) init() {
	tx.Coolroom = NewCoolroomClient(tx.config)
	tx.CoolroomType = NewCoolroomTypeClient(tx.config)
	tx.DeceasedReceive = NewDeceasedReceiveClient(tx.config)
...
// applies a query, for example: Coolroom.QueryXXX(), the query will be executed
