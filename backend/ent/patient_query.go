...
func (pq *PatientQuery) QueryDeceasedreceives() *DeceasedReceiveQuery {
	query := &DeceasedReceiveQuery{config: pq.config}
...
func (pq *PatientQuery) WithDeceasedreceives(opts ...func(*DeceasedReceiveQuery)) *PatientQuery {
	query := &DeceasedReceiveQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withDeceasedreceives = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withDeceasedreceives; query != nil {
		...
		query.Where(predicate.DeceasedReceive(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.DeceasedreceivesColumn, fks...))
		}))
