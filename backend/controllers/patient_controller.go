...
func (ctl *PatientController) register() {
	users := ctl.router.Group("/patients")

	users.GET("", ctl.ListPatient)

	// CRUD
	users.POST("", ctl.CreatePatient)
	users.GET(":id", ctl.GetPatient)
	users.PUT(":id", ctl.UpdatePatient)
	users.DELETE(":id", ctl.DeletePatient)
}
